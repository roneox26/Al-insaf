...
def admin_edit_collections():
    ...
    # Get filter parameters
    collection_type = request.args.get('collection_type', 'loan')
    customer_filter = request.args.get('customer', '').strip()
    from_date = request.args.get('from_date', '').strip()
    to_date = request.args.get('to_date', '').strip()
    ...
    if from_date:
        try:
            start_date = datetime.strptime(from_date, '%Y-%m-%d')
            loan_query = loan_query.filter(LoanCollection.collection_date >= start_date)
        except ValueError:
            pass
    if to_date:
        try:
            end_date = datetime.strptime(to_date, '%Y-%m-%d').replace(hour=23, minute=59, second=59)
            loan_query = loan_query.filter(LoanCollection.collection_date <= end_date)
        except ValueError:
            pass
    loan_collections = loan_query.order_by(LoanCollection.collection_date.desc()).all()
    
    # Get saving collections
    saving_query = SavingCollection.query
    if customer_filter:
        saving_query = saving_query.join(Customer).filter(Customer.name.like(f'%{customer_filter}%'))
    if from_date:
        try:
            start_date = datetime.strptime(from_date, '%Y-%m-%d')
            saving_query = saving_query.filter(SavingCollection.collection_date >= start_date)
        except ValueError:
            pass
    if to_date:
        try:
            end_date = datetime.strptime(to_date, '%Y-%m-%d').replace(hour=23, minute=59, second=59)
            saving_query = saving_query.filter(SavingCollection.collection_date <= end_date)
        except ValueError:
            pass
    saving_collections = saving_query.order_by(SavingCollection.collection_date.desc()).all()
    
    return render_template('admin_edit_collections.html',
                         loan_collections=loan_collections,
                         saving_collections=saving_collections,
                         collection_type=collection_type,
                         customer_filter=customer_filter,
