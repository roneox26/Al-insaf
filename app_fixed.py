...
def add_loan():
    ...
    if request.method == 'POST':
        try:
            ...
            db.session.commit()
            flash(f'লোন দেওয়া হয়েছে! পরিমাণ: ৳{amount}, মোট: ৳{interest_amount}, যোগ করা?মোট: ৳{service_charge}, কল্যাণ ফি: ৳{welfare_fee}, আবেদন ফি: ৳{application_fee}, মোট: ৳{total_with_interest}', 'success')
...
def add_customer():
    if request.method == 'POST':
        try:
            ...
            db.session.commit()
            flash(f'নামসব যোগ করাসব নামনাম! আবেদন ফি: ৳{admission_fee}', 'success')
            return redirect(url_for('manage_customers'))
        except Exception as e:
            db.session.rollback()
            flash(f'Error: {str(e)}', 'danger')
...
def collection():
    ...
    if request.method == 'POST':
        ...
        if saving_amount > 0:
            msg.append(f'নামমোট: ৳{saving_amount}')
...
def collect_loan():
    ...
    try:
        ...
        flash(f'যোগ করা{amount} যোগ করানামনাম নামনাম! ?মোট: ৳{customer.remaining_loan}', 'success')
        return redirect(url_for('loan_collection'))
    except Exception as e:
        db.session.rollback()
        flash(f'Error: {str(e)}', 'danger')
...
def collect_saving():
    ...
    try:
        ...
        flash(f'যোগ করা{amount} সংগ্রহ করা হয়েছে!', 'success')
        return redirect(url_for('saving_collection'))
    except Exception as e:
        db.session.rollback()
        flash(f'Error: {str(e)}', 'danger')
    ...
    if request.method == 'POST':
        try:
            ...
            if customer.savings_balance < amount:
                flash(f'?যোগ করানামসব নাম! নাম?মোট: ৳{customer.savings_balance}', 'danger')
                return redirect(url_for('manage_withdrawals'))
            ...
            db.session.add(withdrawal)
            db.session.commit()
            flash(f'?{amount} যোগ করাসব নামনাম!', 'success')
            return redirect(url_for('manage_withdrawals'))
        except Exception as e:
            db.session.rollback()
            flash(f'Error: {str(e)}', 'danger')
...
def due_report_export():
    ...
    writer.writerow(['নামনাম', 'নামসব সব', 'নাম', 'নাম', 'নামনাম', 'নামনাম নাম?', 'নাম নামনাম', 
                     'সক্রিয়?', 'নামনাম নামনাম', 'নাম সক্রিয়', 'যোগ করানাম?', 'নাম নাম?', 'নামসব নাম?'])
...
def complete_followup(id):
    ...
    db.session.commit()
    flash('নাম-সব যোগ করানামসব!', 'success')
...
def manage_notes():
    ...
    if request.method == 'POST':
        ...
        db.session.add(note)
        db.session.commit()
        flash('নাম যোগ করাসব নামনাম!', 'success')
        return redirect(url_for('manage_notes'))
...
def admin_settings():
    ...
    if request.method == 'POST':
        ...
        if action == 'change_email':
            ...
            if not bcrypt.check_password_hash(current_user.password, password):
                flash('নামযোগ করাসব!', 'danger')
                return redirect(url_for('admin_settings'))
            
            if User.query.filter(User.email == new_email, User.id != current_user.id).first():
                flash('সব নামসব ?যোগ করানামনাম নামসব!', 'danger')
                return redirect(url_for('admin_settings'))
            
            current_user.email = new_email
            db.session.commit()
            flash('নামসব যোগ করাযোগ করানামসব!', 'success')
            return redirect(url_for('admin_settings'))
            ...
            if not bcrypt.check_password_hash(current_user.password, current_password):
                flash('যোগ করাসবযোগ করাসব!', 'danger')
                return redirect(url_for('admin_settings'))
            
            if new_password != confirm_password:
                flash('নাম? নামযোগ করানাম? সব!', 'danger')
                return redirect(url_for('admin_settings'))
            
            if len(new_password) < 6:
                flash('নামযোগ করানামনাম ? যোগ করাসব নাম!', 'danger')
                return redirect(url_for('admin_settings'))
            
            current_user.password = bcrypt.generate_password_hash(new_password).decode('utf-8')
            db.session.commit()
            flash('নামযোগ করানামনাম ?যোগ করানামসব!', 'success')
            return redirect(url_for('admin_settings'))
    ...
    if request.method == 'POST':
        try:
            ...
            if action == 'add_customer':
                ...
                flash(f'নতুন Customer "{name}" যোগ করাসব করা হয়েছে!', 'success')
                return redirect(url_for('import_old_data'))
                ...
                if not customer_id:
                    flash('Customer ?যোগ করানাম!', 'danger')
                    return redirect(url_for('import_old_data'))
                ...
                db.session.commit()
                flash(f'নামনাম Collection নাম করা হয়েছে! মোট: ৳{loan_amount}, নামমোট: ৳{saving_amount}', 'success')
                return redirect(url_for('import_old_data'))
